...
const uri = `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASS}@cluster0.kxsvn49.mongodb.net/?appName=Cluster0`;
...
async function run() {
  try {
    ...
    app.post('/api/products', async (req, res) => {
      try {
        const { name, image, price, oldPrice } = req.body;
        if (!name || !image || price == null || oldPrice == null) {
          return res.status(400).json({ error: 'All fields (name, image, price, oldPrice) are required' });
        }
        const product = { name, image, price, oldPrice, createdAt: new Date() };
    ...
    app.put('/api/products/:id', async (req, res) => {
      try {
        ...
        const updatedProduct = { name, image, price, oldPrice, updatedAt: new Date() };
...
app.get('/api/moderators', async (req, res) => {
    ...
    app.get('/api/user/role', async (req, res) => {
        ...
        // Since we don't have Firebase Admin SDK, we'll assume the token is valid
        // In a production environment, you should verify the token using Firebase Admin SDK
        const decodedToken = { uid: req.query.uid }; // Mock decoded token; replace with actual token verification if possible
        if (!req.query.uid) {
